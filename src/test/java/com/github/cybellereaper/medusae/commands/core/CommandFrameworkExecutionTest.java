...
class CommandFrameworkExecutionTest {
    ...
    @Test
    void reportsMalformedAndMissingRequiredNumericOptions() {
        CommandFramework framework = new CommandFramework();
        framework.registerCommands(new NumericCommand());

        assertThrows(ResolutionException.class, () -> framework.execute(new CommandInteraction(
                "numeric", CommandType.CHAT_INPUT, null, null,
                Map.of("count", new CommandOptionValue("x", 3)),
                null, Map.of(), false, "g1", "u1", Set.of(), Set.of(),
                null, null, null, null, null, null
        ), response -> {}));
    ...
    @Command("numeric")
    static final class NumericCommand {
        @Execute
        void root(@Name("count") int count) {
        }
    }
