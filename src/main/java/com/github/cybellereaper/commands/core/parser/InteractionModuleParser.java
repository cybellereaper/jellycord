...
public final class InteractionModuleParser {
    ...
    private static HandlerMarker detect(Method method) {
        if (method.isAnnotationPresent(ButtonHandler.class)) return new HandlerMarker(InteractionHandlerType.BUTTON, method.getAnnotation(ButtonHandler.class).value());
        if (method.isAnnotationPresent(StringSelectHandler.class)) return new HandlerMarker(InteractionHandlerType.STRING_SELECT, method.getAnnotation(StringSelectHandler.class).value());
        if (method.isAnnotationPresent(UserSelectHandler.class)) return new HandlerMarker(InteractionHandlerType.USER_SELECT, method.getAnnotation(UserSelectHandler.class).value());
        if (method.isAnnotationPresent(RoleSelectHandler.class)) return new HandlerMarker(InteractionHandlerType.ROLE_SELECT, method.getAnnotation(RoleSelectHandler.class).value());
        if (method.isAnnotationPresent(MentionableSelectHandler.class)) return new HandlerMarker(InteractionHandlerType.MENTIONABLE_SELECT, method.getAnnotation(MentionableSelectHandler.class).value());
        if (method.isAnnotationPresent(ChannelSelectHandler.class)) return new HandlerMarker(InteractionHandlerType.CHANNEL_SELECT, method.getAnnotation(ChannelSelectHandler.class).value());
        if (method.isAnnotationPresent(ModalHandler.class)) return new HandlerMarker(InteractionHandlerType.MODAL, method.getAnnotation(ModalHandler.class).value());
        return null;
    }
